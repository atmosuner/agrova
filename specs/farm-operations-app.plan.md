...
>
> **Progress (M6):** M6-01..M6-08 ✅ in repo: push stack + prefs + **`docs/operations/vapid-and-web-push.md`** (`pnpm vapid:keys`, Supabase secrets, redeploy) + **`web-push-fanout`** recipient routing in **`supabase/functions/_shared/web-push-fanout-recipients.ts`** (owner ↔ assignee: task created/duplicated → assignee; status changes → other owners; reassign → previous+new+owners; issues → other owners) + **`supabase/functions/web-push-fanout/IDEMPOTENCY.md`** (duplicate `notifications` insert = `23505` skip) + `invoke-web-push-fanout.test.ts` + `notification-prefs` + `web-push-fanout-recipients` tests + **M6-ω** push KPI in staging. **E2E push** to a real subscription remains manual smoke in deployed env.
>
...
>
> **Progress (M8):** Shipped: export, anonymize, `/offline`, `/how-to-install`, a11y/axe, contrast doc, **nightly E2E** workflow, `e2e/m8-10-critical-path-smoke.spec.ts` (seven **unauthenticated** shell smokes) + a11y routes, **owner Settings → Log out** (shared `signOutAndClearLocalData`, tests in `src/features/auth/logout.test.ts`), **password:** `/forgot-password` + `/reset-password` (email self-service; set `VITE_SITE_URL` and Supabase Auth **Redirect URLs** for `/reset-password`), **change password** on owner Settings + worker `/m/profile`, owner **Set password** for crew (`supabase/functions/set-worker-password`, deploy with `--no-verify-jwt`); `@device.agrova.app` workers have no inbox reset (owner sets password or in-app change). **Pending:** Lingui tr catalog completion, Lighthouse/PWA in CI, **authed** seven-flow E2E + staging, **M8-11/12** domain + launch.
>
...
**Description:** `/people` page with list + "New person" modal. Fields: full_name, phone (E.164 via Zod), role select. **Add** calls Edge Function `create-team-person` (owner JWT in `Authorization`; deploy with `--no-verify-jwt`): inserts `people`, creates Supabase Auth user (`w{uuid}@device.agrova.app`) with **owner-set password**, links `auth_user_id`, clears setup tokens — client copies **device login e-mail** to clipboard. Row “Create setup link” was removed in favor of password sign-in; legacy `/setup/{token}` + `claim-setup-token` remain for older flows.
**Acceptance criteria:**
